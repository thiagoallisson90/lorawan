...
def dump(pso, filename='pso04.txt'):
	with open(f'/home/thiago/Documentos/Doutorado/Simuladores/ns-3-allinone/ns-3.38/scratch/pso-fuzzy/{filename}', 'a') as file:
		file.write(str(pso))

def dump_json(data, filename='pso04.json'):
	with open(f'/home/thiago/Documentos/Doutorado/Simuladores/ns-3-allinone/ns-3.38/scratch/pso-fuzzy/{filename}', 'a') as file:
		file.write(json.dumps(data))

def dump_all(data01, data02, filename01='pso04.txt', filename02='pso04.json'):
	executor = concurrent.futures.ThreadPoolExecutor()
	results = [executor.submit(dump, data01, filename01),
						 executor.submit(dump_json, data02, filename02)]
	concurrent.futures.wait(results)
	executor.shutdown()	
...
def generate_random_coords(min_val, max_val, min_dist):
	coords = np.zeros(9)
	...
	while flag or coords[8] - coords[6] < min_dist:
		...
		coords[7] = np.random.uniform(coords[6], max_val)
		coords[6:9].sort()
		flag = False
...
def init_pos(VarMin, VarMax, min_dist):
	SNR_coords = generate_random_coords(VarMin[0], VarMax[0], min_dist[0])
	TP_coords = generate_random_coords(VarMin[9], VarMax[9], min_dist[1])
	SF_coords = generate_random_coords(VarMin[19], VarMax[19], min_dist[2])
...
def apply_lim_pos_vars(newP, min_val, max_val, min_dist):
	newP[0] = min_val
	newP[8] = max_val
	...
	newP[0:3].sort()
	while newP[2] - newP[0] < min_dist:
		newP[2] = np.random.randint(min_val, max_val)
		newP[0:3].sort()

	newP[6:9].sort()
	while newP[8] - newP[6] < min_dist:
		newP[6] = np.random.randint(min_val, max_val)
		newP[6:9].sort()

	newP[3:6].sort()
	while newP[3] > newP[2]:
		newP[3]	= np.random.randint(newP[0], newP[2])
		newP[3:6].sort()

	newP[3:6].sort()
	while newP[5] < newP[6]:
		newP[5] = np.random.randint(newP[6], max_val)
		newP[3:6].sort()

	newP[3:6].sort()
	while newP[5] - newP[3] < min_dist:
		newP[3] = np.random.randint(newP[0], newP[2])
		newP[4] = np.random.randint(newP[3], max_val)
		newP[5] = np.random.randint(newP[6], max_val)
		newP[3:6].sort()
...
def PSO(problem, params):
	...
	BestCostsOfIter = np.zeros(MaxIt)
	
	for i in range(nPop):
		particle[i] = {'Position': init_pos(VarMin, VarMax, [4, 2, 1.5]),
		...
		if particle[i]['Best']['Cost'] < GlobalBest['Cost']:
			GlobalBest['Position'] = particle[i]['Best']['Position'].copy()
    ...
    'pop': particle, 
    'BestOfIter': {
	    'Position': GlobalBest['Position'].copy(),
    ...
    'BestCostsOfIter': GlobalBest['Cost'] 
  }
	dump_all(out, out_json)

	if ShowIterInfo:
		print(f'Melhor custo da iteração 0 = {GlobalBest["Cost"]}')
	...
	for it in range(MaxIt):	
		for i in range(nPop):
			...
			particle[i]['Velocity'] = (w * particle[i]['Velocity'] +
																c1 * np.random.uniform(0, 1, nVar) * (particle[i]['Best']['Position'] - particle[i]['Position']) +
																c2 * np.random.uniform(0, 1, nVar) * (GlobalBest['Position'] - particle[i]['Position']))

			particle[i]['Velocity'] = apply_lim_vel(particle[i]['Velocity'], MinVel, MaxVel)
			# *****************************************************************************************************************
			# Atualizar posições
			newPositions = particle[i]['Position'] + particle[i]['Velocity']
			
			particle[i]['Position'][0:9] = apply_lim_pos_vars(newPositions[0:9], VarMin[0], VarMax[0], 4.0) # SNR
			particle[i]['Position'][9:18] = apply_lim_pos_vars(newPositions[9:18], VarMin[9], VarMax[9], 2.0) # TP
			particle[i]['Position'][18:27] = apply_lim_pos_vars(newPositions[18:27], VarMin[18], VarMax[18], 1.5) # SF
		...
		particle_sort = sorted(particle, key=lambda p: p['Cost'])
		BestCosts[it] = GlobalBest['Cost']
		BestCostsOfIter[it] = particle_sort[0]['Cost']
		w *= wdamp
			
		if ShowIterInfo:
			print(f'Melhor custo global na iteração {it + 1} = {BestCosts[it]}')
			print(f'Melhor custo da iteração {it + 1} = {particle_sort[0]["Cost"]}')
    
		out = {
      'Iteration': (it+1),
      'pop': particle, 
      'BestOfIter': {
	      'Position': particle_sort[0]['Position'].copy(),
	      'Cost': particle_sort[0]['Cost']
			},
      'BestSol': GlobalBest, 
      'BestCosts': BestCosts,
      'BestCostsOfIter': BestCostsOfIter 
    }
		out_json = {
      'Iteration': (it+1),
      'BestCosts': BestCosts.tolist(),
      'BestCostsOfIter': BestCostsOfIter.tolist() 
    }
		dump_all(out, out_json)
					
	executor = concurrent.futures.ThreadPoolExecutor()
	results = [executor.submit(dump, GlobalBest, 'best-global-pso04.txt'),
						 executor.submit(dump, BestCosts, 'best-costs-pso04.txt'),
						 executor.submit(dump, BestCostsOfIter, 'costs-iters-pso04.txt')]
	concurrent.futures.wait(results)
	executor.shutdown()	
