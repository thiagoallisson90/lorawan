...
NS_LOG_COMPONENT_DEFINE ("AdrFuzzy");

NS_OBJECT_ENSURE_REGISTERED (AdrFuzzy);

TypeId AdrFuzzy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::AdrFuzzy")
    .SetGroupName ("lorawan")
    .AddConstructor<AdrFuzzy> ()
    ...
    .AddAttribute ("MultipleGwCombiningMethod",
                   ...
                   EnumValue (AdrFuzzy::AVERAGE),
                   MakeEnumAccessor (&AdrFuzzy::tpAveraging),
                   MakeEnumChecker (AdrFuzzy::AVERAGE,
                                    "avg",
                                    AdrFuzzy::MAXIMUM,
                                    "max",
                                    AdrFuzzy::MINIMUM,
    ...
    .AddAttribute ("MultiplePacketsCombiningMethod",
                   ...
                   EnumValue (AdrFuzzy::AVERAGE),
                   MakeEnumAccessor (&AdrFuzzy::historyAveraging),
                   MakeEnumChecker (AdrFuzzy::AVERAGE,
                                    "avg",
                                    AdrFuzzy::MAXIMUM,
                                    "max",
    ...
    .AddAttribute ("HistoryRange",
                   ...
                   IntegerValue (4),
                   MakeIntegerAccessor (&AdrFuzzy::historyRange),
                   MakeIntegerChecker<int> (0, 100))
    .AddAttribute ("ChangeTransmissionPower",
                   "Whether to toggle the transmission power or not",
                   BooleanValue (true),
                   MakeBooleanAccessor (&AdrFuzzy::m_toggleTxPower),
                   MakeBooleanChecker ())
    .AddAttribute ("Run",
                   "Run Number",
                   IntegerValue (1),
                   MakeIntegerAccessor (&AdrFuzzy::m_run),
                   MakeIntegerChecker<int> (1, 100))
  ;
  return tid;
}
...
AdrFuzzy::AdrFuzzy ()
{
  std::string path = "/home/thiago/Documentos/Doutorado/Simuladores/ns-3-allinone/ns-3.38/src/lorawan/examples/";
  std::string filename = path + "fadr.fll";
...
void AdrFuzzy::OnReceivedPacket (Ptr<const Packet> packet,
...
void
AdrFuzzy::BeforeSendingReply (Ptr<EndDeviceStatus> status,
                                  Ptr<NetworkStatus> networkStatus)
{
...
void AdrFuzzy::OnFailedReply (Ptr<EndDeviceStatus> status,
                                  Ptr<NetworkStatus> networkStatus)
{
  NS_LOG_FUNCTION (this->GetTypeId () << networkStatus);
}

void AdrFuzzy::AdrImplementation (uint8_t *newDataRate,
                                      uint8_t *newTxPower,
                                      Ptr<EndDeviceStatus> status)
{
...
uint8_t AdrFuzzy::SfToDr (uint8_t sf)
{
  switch (sf)
    {
    case 12:
...
double AdrFuzzy::RxPowerToSNR (double transmissionPower)
{
...
double AdrFuzzy::GetMinTxFromGateways (EndDeviceStatus::GatewayList gwList)
{
...
//Get the maximum received power (it considers the values in dB!)
double AdrFuzzy::GetMaxTxFromGateways (EndDeviceStatus::GatewayList gwList)
{
  EndDeviceStatus::GatewayList::iterator it = gwList.begin ();
...
double AdrFuzzy::GetAverageTxFromGateways (EndDeviceStatus::GatewayList gwList)
{
  double sum = 0;
...
double
AdrFuzzy::GetReceivedPower (EndDeviceStatus::GatewayList gwList)
{
  switch (tpAveraging)
    {
    case AdrFuzzy::AVERAGE:
...
double AdrFuzzy::GetMinSNR (EndDeviceStatus::ReceivedPacketList packetList,
                                int historyRange)
{
  double m_SNR;
...
double AdrFuzzy::GetMaxSNR (EndDeviceStatus::ReceivedPacketList packetList,
                                int historyRange)
{
  double m_SNR;
...
double AdrFuzzy::GetAverageSNR (EndDeviceStatus::ReceivedPacketList packetList,
                                    int historyRange)
{
  double sum = 0;
  double m_SNR;
...
int AdrFuzzy::GetTxPowerIndex (int txPower)
{
  if (txPower >= 16)
    {
      return 0;
